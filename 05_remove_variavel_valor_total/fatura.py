def gera_fatura(dados_demonstrativo, obras):
    ...
    valor_total = calcula_valor_total(dados_demonstrativo["performances"])
    resultado += f"Valor a pagar é de {brl(valor_total / 100)}\n"
    resultado += f"Você ganhou {calcula_creditos_totais(dados_demonstrativo['performances'])} créditos\n"
    return resultado
...
def calcula_creditos_totais(performances):
    resultado = 0
...
def calcula_valor_total(performances):
    resultado = 0
    for performance in performances:
